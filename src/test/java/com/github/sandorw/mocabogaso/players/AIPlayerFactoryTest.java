...
public final class AIPlayerFactoryTest {
    ...
    public void multiThreadedAMAFAIPlayerTest() {
        game.addPlayer("Player 1", AIPlayerFactory.getNewMultiThreadedAMAFAIPlayer(gameState, 50));
        game.addPlayer("Player 2", AIPlayerFactory.getNewMultiThreadedAMAFAIPlayer(gameState, 50));
        game.playGame();
        assertTrue(game.isGameOver());
        assertFalse(game.getGameResult().isTie());
    }
